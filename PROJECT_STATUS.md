...
- **2026-08-03 — Settlements deployed to production + Turso view-DDL gotcha.** Applied `migrations/0002_add_settlements.sql` to the production Turso DB and deployed backend (Render, auto-deploy on push) and frontend (Cloudflare Worker via the new committed `frontend/wrangler.jsonc` + `npm run deploy`, which builds with `VITE_API_URL=https://recibosplit-api.onrender.com` baked in — no more reconstructing deploy config from the live bundle). **CRITICAL GOTCHA for every future migration:** the pyturso embedded-replica sync engine propagates `CREATE TABLE` to the remote but **silently drops `DROP VIEW` / `CREATE VIEW` statements on `push()`** — the local replica looks correct, the push reports success, and the remote keeps the old view definitions (surfaced as a production 500 on `GET /balances` after deploy: `no such column: total_settled_sent_all_events`). Verified empirically: statement-by-statement `execute()` + `commit()` + `push()` doesn't help either. The fix is to run view DDL **directly against the remote** via Turso's HTTP SQL API (`POST https://<db-host>/v2/pipeline` with the auth token) and then verify through a **brand-new** replica file pulled from remote — never trust the migrating replica's own state as proof the remote changed. Table DDL and data writes sync fine.
- **2026-08-01 — Settlements: record debt payments between participants.** New `settlements` table (event-scoped so each payment has an unambiguous currency; `migrations/0002_add_settlements.sql` mirrors `schema.sql` exactly) with the balance math staying in the SQL views only: `event_balances` gains `total_settled_sent`/`total_settled_received` and `net_balance = paid - consumed + settled_sent - settled_received`; `overall_balances` sums the new columns and inherits settlements automatically. New endpoints: `POST/GET /events/{id}/settlements`, `GET /settlements` (cross-event, with event name), `DELETE /settlements/{id}`; `delete_participant` guard extended (409 if involved in a settlement), `delete_event` and `clear_all_data` clean settlements up. Balances tab UI: per-event "Record payment" form (payer/recipient/amount/optional note), per-debtor "Settle up" prefill button (suggests min(|debtor net|, creditor net) toward the largest creditor), payments history with delete, and an "incl. payments" hint on net cells — full EN/ES i18n. Fresh-context review caught one CRITICAL before commit: `delete_event` didn't delete settlements, leaving orphaned rows invisible to the whole API that permanently blocked participant deletion — fixed with an explicit `DELETE FROM settlements WHERE event_id = ?` plus a regression test. MCP server balance-tool docstrings updated to the new formula. Suite: 137 passed (121 → 137, 16 new in `tests/test_settlements.py`). **Pending: apply `migrations/0002_add_settlements.sql` to the production Turso DB before deploying** — the new table/views don't exist there yet.

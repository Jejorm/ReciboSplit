...
- **NOT COMMITTED — INCIDENT:** the working copy is no longer a git repository. `.git`, `.gitignore`, and `.mcp.json` are missing (nearly all files share mtime 2026-07-17 12:06 — the folder appears to have been copied/recreated dropping those dotfiles). All Phase 1/2 commit history is gone locally. Recovery (Time Machine, a surviving clone, or fresh `git init`) is a user decision; nothing can be committed until then.
